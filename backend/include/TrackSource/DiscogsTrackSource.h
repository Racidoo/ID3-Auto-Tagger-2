...
class DiscogsTrackSource : public ITrackSource {
  private:
    std::shared_ptr<Discogs::SearchResult> track;

  public:
    explicit DiscogsTrackSource(std::shared_ptr<Discogs::SearchResult> _track)
    ...
    std::shared_ptr<Discogs::SearchResult> get_track() const;
};