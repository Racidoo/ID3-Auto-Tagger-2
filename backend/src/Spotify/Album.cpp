...
Album::Album(const std::string &_id, const std::string &_name, State _state,
    ...
    : IAlbum(_id, _state, _mediaService), name(_name), imageProvider(_imageURL),
      ...
      releaseDate(_releaseDate), artists(_artists) {}

std::optional<std::string> Album::get_name() const { return name; }
std::optional<std::vector<std::byte>> Album::get_image() {
    return imageProvider.get_image();
}
IAlbum::album_type_t Album::get_type() const { return type; }
std::optional<std::string> Album::get_artist() const {
    return vecToStr(artists);
}
std::optional<std::vector<std::byte>> Album::get_artistImage() {
    if (artists.empty()) {
        return std::nullopt;
    }
    return artists[0]->get_image();
}
std::optional<std::size_t> Album::get_year() const { return year; }
std::size_t Album::get_totalTracks() const { return totalTracks; }
const std::string Album::get_releaseDate() const { return releaseDate; }
std::optional<std::string> Album::get_label() const { return label; }
std::optional<std::string> Album::get_copyright() const { return copyright; }
...
void Album::set_label(const std::optional<std::string> &_label) {
    label = _label;
}
void Album::set_copyright(const std::optional<std::string> &_copyright) {
