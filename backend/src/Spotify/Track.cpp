...
Track::Track(const std::string &_id, const std::string &_name,
             MetadataState _state, const unsigned int _discNumber,
             const unsigned long _durationMs, const bool _explicitContent,
             const unsigned int _trackNumber, const Album &_album,
             const std::vector<Artist> &_artists)
    : QueryObject(_id, _name, _state, ""), discNumber(_discNumber),
