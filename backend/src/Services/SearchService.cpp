...
SearchService::search(std::shared_ptr<ITrack> _track,
    ...
    if (_track->get_name().empty() && _track->get_artist().empty() &&
        _track->get_id().empty())
        return {};

    std::string query;

    if (_track->get_name().empty())
        query = _track->get_id();
    else if (_track->get_artist().empty())
        query = _track->get_name();
    else
        query = _track->get_artist() + " - " + _track->get_name();

    if (!_track->get_albumName().empty())
        query += " - " + _track->get_albumName();
...
SearchService::search(const std::string &_query,
    ...
    for (auto &&aggTrack : agg) {
        std::cout << aggTrack.primary->get_name() << " - "
                  << aggTrack.primary->get_artist() << "\nIDs:" << std::endl;
        for (auto &&source : aggTrack.sources) {
            std::cout << "\t" << source->get_id() << std::endl;
        }
    }
